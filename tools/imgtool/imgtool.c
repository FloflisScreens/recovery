...
int decompressKernel (unsigned char *kernel, int size)
{
   ...
   // and then invoke gunzip - because we have the magic, it will take care of that.
    ...
    int outputFd = -1;
    int dtFd = -1;
    system ("mkdir ./extracted 2> /dev/null");
...
char *makeImg (char *imgName, char *kernelName, char *ramdiskName, uint32_t Addr, char *CmdLine)
{
	...
	if (k < 0) { perror (kernelName); return 1001; }
	...
	if (rd < 0) { perror (ramdiskName); return 1002; }
	rc = fstat(rd, &stbuf);
	...
	strncpy (bih.magic, BOOT_MAGIC, BOOT_MAGIC_SIZE);
	...
	strcpy (bih.name, "1490613711");
	memset (bih.cmdline, '\0', sizeof(bih.cmdline));
	if (CmdLine)
	{
		strncpy(bih.cmdline, (unsigned char *) CmdLine,BOOT_ARGS_SIZE-1);
	}
	else
	strcpy (bih.cmdline, (unsigned char *) "androidboot.hardware=flounder androidboot.selinux=disabled");
...
int main (int argc, char **argv)
{
	...
	for (i = 1; i <= argc - 1; i++)
	{
	  ...
	  if (strncasecmp(argv[i], "extract", 7) == 0) extract++;
	...
	if (strncmp(mmapped,BOOT_MAGIC, strlen(BOOT_MAGIC)) == 0)
	{
		...
		if (extract)
		{
			fprintf (stderr,"Extracting contents to directory: %s\n", EXTRACTED_DIR);
			mkdir (EXTRACTED_DIR, 0777); // yeah, I know. Insecure. Bleh


			decompressKernel (mmapped +  1 * bih->page_size, bih->kernel_size);
	...
	
	{
		...
		if (simg_after_all){
    		struct simg_header *si = (struct simg_header *) simg_after_all;
                ...
                printf("Sparse image v%d.%d detected (in container at offset 0x%x)\n", si->version_major, si->version_minor, simg_after_all - mmapped );
                printf("%d blocks of %d bytes compressed into %d chunks (%lld%% compressed)\n",
                        si->num_blocks, si->block_size, si->num_chunks,
